...
oldkeysenable = RestrictKeysForKbCheck([kb.escKey, kb.spaceKey, kb.downKey, kb.rightKey]);
...
stim_list_file_base = 'Stimlist_Practice_Ba_Da.txt';
...
num_trials_each = 1; % How many times each stimulus will be presented
...
repeat_number = 1;
results_file_base = [SubjectCode '_categorization_practice_' num2str(repeat_number) '.txt'];
...
Screen('DrawTextures', window, imageTexture2, [], dstRects(:,2));
Screen('DrawText', window, 'Ba', screenXpixels/4, screenYpixels/7);
Screen('DrawText', window, 'Da', (3/4)*(screenXpixels), screenYpixels/7);
