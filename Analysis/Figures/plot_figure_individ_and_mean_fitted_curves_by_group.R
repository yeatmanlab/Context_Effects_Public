...
group_levels <- c("Dyslexic", "Control", "Group Means")
...
psychometrics <- psychometrics %>% 
  filter(threshold <= 7 & threshold >= 1)
...
psych_subj_agg$plot_group <- with(psych_subj_agg, interaction(subject_id,type))
...
psych_group_ribbon <- psych_subj_agg %>%
  group_by(group, type) %>%
  summarise(read=mean(read, na.rm=TRUE),
...
endpoints <- range(na.omit(psychometrics$read))
c_range <- diff(endpoints)
group_vals <- psych_subj_agg %>% group_by(group) %>% summarise(read=mean(read))
group_vals <- (group_vals$read - endpoints[1]) / c_range
...
fig <- ggplot(data=logist_subj_agg, mapping=aes(x=x, y=y, group=plot_group)) +
  geom_line(aes(colour=read), size=0.75, alpha=1) +
  geom_ribbon(aes(ymin=y_lower, ymax=y_upper, fill=read), alpha=0.2, size=0) +
  ...
  labs(title="", colour="Reading\nScore", fill="Reading\nScore") +
  xlab("Continuum Step") + ylab("") +
  facet_grid(type ~ group, labeller=duration_labeller, switch="y")
