...
lmfit <- lmer(log_RT ~ type*read + age_at_testing + (0 + type|subject_id) + (1|step), clean_data)
...
# There is a significant interaction of continuum type and reading skill- better readers have a greater effect of continuum type
...
ggplot(clean_data, aes(step, RT, colour = group, fill = group))+
  geom_smooth(method = "gam", formula = y ~ poly(x,2))+
  facet_wrap(~type)

ggplot(clean_data, aes(step, RT, colour = type, fill = type))+
  geom_smooth(method = "gam", formula = y ~ poly(x,2))+
  facet_wrap(~group)

ggplot(df_sum_step, aes(step, mu))+
  geom_smooth(method = "gam", formula = y ~ poly(x,2), aes(group = subject_id, colour = type), se = FALSE)+
  facet_wrap(group~type)

# A linear model says....
lmfit <- lmer(log_RT ~ poly(step,2)*read + (1|subject_id) + (1|step), clean_data)
summary(lmfit)
...
# Individual 2nd order polynomial fits don't appear to have a significant interaction with reading skill
summary(lmer(coef ~ type*read +(1|subject_id), fits))
