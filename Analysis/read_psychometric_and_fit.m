function read_psychometric_and_fit(subject_id, type,width)
% arguments%%%%%
%
% subject_id (str)
% continuum (str)
    ...
    % Read in table
    filename = ['../Results/Psychometrics/Raw/Psychometrics_',type, '_', subject_id, '.csv'];
    T = readtable(filename);

    A_psych = T;
    
    levels = 1:7;
    presentations = A_psych.count';

    %% Get the slope of the first psychometric
    labelled = table2array(A_psych(:,4))' .* presentations;
...
options.priors{2} = @(x) ((x.*Cfactor)>=widthmin).*((x.*Cfactor)<=2*widthmin).*(.5-.5*cos(pi.*((x.*Cfactor)-widthmin)./widthmin))...
    ...
    header = 'SubjectID,width,type,threshold,slope,lapse,guess,eta,deviance,residuals,loglikelihood';
    fid = fopen(['../Results/Psychometrics/Fit/Fit_', ...
        type, '_', subject_id, '_', num2str(width),'.csv'], 'w');

    fprintf(fid, '%s\n', header);
    fprintf(fid, '%s,%f,%s,%f,%f,%f,%f,%f,%f,%f,%f\n', subject_id,width,type,paramsA(1),paramsA(2),paramsA(3),paramsA(4),paramsA(5),Adev,residualsA,loglikelihoodA);
