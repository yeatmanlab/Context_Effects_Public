...
parfor j = 1:length(sid_list)
    ...
    for w = 1:length(width_vec)
        try
        out1  = Cross_validation_k10(subject_id,1, width_vec(w));
            warning('Oops, something went wrong.')

        out2  = Cross_validation_k10(subject_id, 2, width_vec(w))
        
        header = 'SubjectID,block,width,p';
        
        
        fid = fopen(['/home/eobrien/bde/Projects/Speech_dist/Results/CV_10/'...
